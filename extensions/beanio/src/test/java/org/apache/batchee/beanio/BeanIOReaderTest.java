...
public class BeanIOReaderTest {
    ...
    public void read() throws Exception {
        final String path = "target/work/BeanIOReader.txt";
        ...
        Batches.waitForEnd(jobOperator, jobOperator.start("beanio-reader", jobParams));

        final int size = StoreItems.ITEMS.size();
        assertEquals(size, 3);
        for (int i = 1; i <= size; i++) {
            final Record record = StoreItems.ITEMS.get(i - 1);
            assertEquals("v" + i + "1", record.getValue1());
            assertEquals("v" + i + "2", record.getValue2());
        }
    }
